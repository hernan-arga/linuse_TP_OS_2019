...
void *musemalloc(uint32_t tamanio, int idSocketCliente) {
	...
	if(list_is_empty(segmentosProceso)){ //Si no tiene ningun segmento se lo creo
		...
		void *pos;
		// todo REVISAR SI HACE FALTA RESTAR EL TAMANIO LIBRE DEL ULTIMO HEAP
...
struct HeapMetadata *leerMetadata(struct Segmento *segmento, struct HeapLista *heapLista){
	...
	if(heapLista->estaPartido == true){
		memcpy(metadata, pos + desplazamientoPrimeraPagina, heapLista->bytesPrimeraPagina);
		...
		memcpy(metadata + heapLista->bytesPrimeraPagina, pos, sizeof(struct HeapMetadata) - heapLista->bytesPrimeraPagina);
	} else {
		memcpy(metadata, pos + desplazamientoPrimeraPagina, sizeof(struct HeapMetadata));
	}
...
struct Segmento *crearSegmento(uint32_t tamanio, int idSocketCliente) {
	...
	double paginas = (double)(tamanio + 2*sizeof(struct HeapMetadata)) / (double)tam_pagina;
	...
	while (paginasNecesarias > 0) {
		...
		if (paginasNecesarias == (int) (ceil(paginas)) || paginasNecesarias == 1) { //Si es la primera pagina o la ultima(tener en cuenta tamaño metadata)
				...
				if (paginasNecesarias == (int) (ceil(paginas))) { //Si es la primera pagina
						nuevoSegmento = asignarPrimeraPaginaSegmento(nuevoSegmento, tamanio, sePartioUnaMetadata);

						//Disminuye el tamaño que se aloco
						int diferenciaTamanios = pconfig->tamanio_pag - sizeof(struct HeapMetadata);
						...
						if (diferenciaTamanios < 0) { //Si se corta la metadata
							...
							if ( ((int)tamanio) < diferenciaTamanios ) { //NO se cortan los datos
								...
								paginasNecesarias -= (ceil)((double)sizeof(struct HeapMetadata) / (double)pconfig->tamanio_pag);
							} else {
								tamanioAlocado -= fabs(pconfig->tamanio_pag - sizeof(struct HeapMetadata));
								//Aca se parten los datos, agrego una pagina
								struct Pagina *paginaExtra = malloc(sizeof(struct Pagina));
								...
								paginasNecesarias -= (ceil)((double)sizeof(struct HeapMetadata) / (double)pconfig->tamanio_pag);
								paginasNecesarias -= (ceil)((double)(tamanio - (sizeof(struct HeapMetadata) - pconfig->tamanio_pag))/ (double)pconfig->tamanio_pag);
							}
...
struct HeapLista *ubicarMetadataYHeapLista(struct Segmento *segmento, int ubicacionHeap, bool isFree, uint32_t size, bool *sePartioUnaMetadata){
	...
	//int indicePrimeraPagina = ubicacionHeap / pconfig->tamanio_pag; // TODO FIXME esto no anda, ubicacionHeap no es un numero, no entiendo que qusiieron hacer
	struct Pagina *pagina = list_get(segmento->tablaPaginas, indicePrimeraPagina);
	...
	if(pconfig->tamanio_pag - desplazamientoPrimeraPagina < sizeof(struct HeapMetadata)){ //Si queda partida
		...
		heap->bytesPrimeraPagina = pconfig->tamanio_pag - desplazamientoPrimeraPagina;
...
struct Segmento *extenderSegmento(struct Segmento *segmento, uint32_t tamanio) {
	...
	*sePartioUnaMetadata = false;
	...
	//TODO ACA HAY QUE CONTEMPLAR la metadata partida
	if( (pconfig->tamanio_pag - (desplazamientoPagina + sizeof(struct HeapMetadata)) - tamanio) < sizeof(struct HeapMetadata)){
		segmento = asignarPrimeraPaginaSegmento(segmento, tamanio, sePartioUnaMetadata);

		bytesAAgregar = tamanio + 2 * sizeof(struct HeapMetadata) - ultimaMetadata->size;
		paginasNecesarias = (int) (ceil((double) bytesAAgregar / (double) pconfig->tamanio_pag));

		/*
		if (tamanio < (pconfig->tamanio_pag - sizeof(struct HeapMetadata))) {
			tamanioAlocado -= (tamanio - sizeof(struct HeapMetadata));
		} else {
			tamanioAlocado -= (pconfig->tamanio_pag
		...
		if (paginasNecesarias == 1) {
			...
			proximaPagina->indiceSwap = -1;
			proximaPagina->numeroFrame = asignarUnFrame();
			proximaPagina->presencia = 1;

			list_add(segmento->tablaPaginas, proximaPagina);

			//Agrego primera metadata donde arranca el frame
			struct HeapMetadata *primeraMetadata = malloc(sizeof(struct HeapMetadata));
			primeraMetadata->isFree = true;
			primeraMetadata->size = tamanio;

			//list_add(proximaPagina->listaMetadata, (void*)(0));
			int ubicacionHeap = obtenerIndicePagina(segmento->tablaPaginas, proximaPagina) * pconfig->tamanio_pag;
			ubicarMetadataYHeapLista(segmento, ubicacionHeap, primeraMetadata->isFree, primeraMetadata->size, sePartioUnaMetadata);

			//Agrego la proxima metadata donde termina tamanio
			struct HeapMetadata *proximaMetadata = malloc(sizeof(struct HeapMetadata));
			proximaMetadata->isFree = true;
			proximaMetadata->size = pconfig->tamanio_pag - sizeof(struct HeapMetadata) - tamanio;

			int direccionHeap = obtenerIndicePagina(segmento->tablaPaginas, proximaPagina) * pconfig->tamanio_pag + sizeof(struct HeapMetadata) + tamanio;
			ubicarMetadataYHeapLista(segmento, direccionHeap, proximaMetadata->isFree, proximaMetadata->size, sePartioUnaMetadata);

		}

	} else {
		bytesAAgregar = tamanio + sizeof(struct HeapMetadata) - ultimaMetadata->size;
...
struct Segmento *asignarPrimeraPaginaSegmento(struct Segmento *segmento, int tamanioMetadata, bool *sePartioUnaMetadata) {
	...
	if(list_size(segmento->metadatas) >= 2){
		...
		for(int i = 1; i < list_size(segmento->metadatas) - 1; i++){
			...
			if(heap->isFree == true && siguienteHeap->isFree == true){
				...
				memcpy((void*)(heap->direccionHeap), nuevoMetadata, sizeof(struct HeapMetadata));
			} else {
...
int musecpy(uint32_t dst, void* src, int n, int idSocketCliente) {
	...
	struct Segmento *unSegmento = segmentoQueContieneDireccion(listaSegmentos, dst);
	//struct Pagina *unaPagina = paginaQueContieneDireccion(unSegmento, dst);
	...
	struct HeapMetadata *metadataDst = malloc(sizeof(struct HeapMetadata));
	metadataDst->size = -1;
	struct HeapMetadata *metadataAux = malloc(sizeof(struct HeapMetadata));
	int offset = direccion - unSegmento->baseLogica;
	struct Pagina *pagina;
	void *pos;
	int desplazamientoAMetadata;
	...
	if(unSegmento->esComun == true){ //Recorro los metadatas del segmento buscando la que contenga direccion
		...
		//TODO
/*
		for(int i = 0; i < list_size(unSegmento->tablaPaginas); i++){
			...
			for(int j = 0; j < list_size(metadatas); j++){
				...
				if((i * pconfig->tamanio_pag) + desplazamientoAMetadata + sizeof(struct HeapMetadata) <= offset &&
					...
					pos = obtenerPosicionMemoriaPagina(pagina) + desplazamientoAMetadata + sizeof(struct HeapMetadata);
					break;
...
*/
		...
		if(metadataDst->size == -1){ //No se encontro el heap de destino
			return -1;
		}

		if(metadataDst->size < n){
			return -1;
		}
		...
		int indicePrimeraPagina = offset / pconfig->tamanio_pag;
		struct Pagina *pagina = list_get(unSegmento->tablaPaginas, indicePrimeraPagina);
		int bytesPrimeraPagina = pconfig->tamanio_pag - (offset % pconfig->tamanio_pag);
		int bytesACopiar = n;
		...
		if(bytesACopiar < bytesPrimeraPagina){
			memcpy(obtenerPosicionMemoriaPagina(pagina) + (offset % pconfig->tamanio_pag), src, bytesACopiar);
			bytesACopiar -= bytesACopiar;
		} else{
			memcpy(obtenerPosicionMemoriaPagina(pagina) + (offset % pconfig->tamanio_pag), src, bytesPrimeraPagina);
		...
		while(bytesACopiar > 0){
			...
			if(bytesACopiar >= tam_pagina){
				...
				memcpy(obtenerPosicionMemoriaPagina(proximaPagina), src + (n - bytesACopiar), bytesACopiar);
				bytesACopiar -= bytesACopiar;
		...
		traerPaginasSegmentoMmapAMemoria(offset, n, unSegmento);
		...
		if(indicePrimeraPagina * pconfig->tamanio_pag + (offset % pconfig->tamanio_pag) + n > unSegmento->tamanio){
			return -1;
		}
...
int musefree(int idSocketCliente, uint32_t dir) {
	...
	// metadatas = pagina->listaMetadata; TODO cambiar estrcutura aca
